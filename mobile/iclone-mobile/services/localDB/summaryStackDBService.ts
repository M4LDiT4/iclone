...
class SummaryStackDBService {
  ...
  async getNewLeafIndex(chatId: string): Promise<number> { 
    try{
      ...
      console.error(`Failed to generate new leaf index: ${err}`);
      throw new LocalDBError("Failed to generate new leaf index");
    }
  }
  ...
  async getNewNodeIndex(chatId: string): Promise<number> {
    ...
    return nodes[0].index + 1;
      ...
      console.log(`Failed to generate new node index: ${err}`);
      throw new LocalDBError("Failed to generate new leaf index");
    }
  }
  ...
  async pushSummaryNode(node: RawSummaryData, type: SummaryType): Promise<SummaryNode> {
    try{
      ...
      console.error(`Failed to create a new summary stack node: ${err}`);
      throw new LocalDBError("Failed to create a new summary node");
    }
  }
  ...
  async pushLeafSummary(
    ...
    messageIdList: string[]
      ...
      console.error(`Failed to create a new summary stack leaf: ${err}`);
      throw new LocalDBError("Failed to create a new summary stack leaf");
    }
  }
  ...
  async getStackItems(chatId: string): Promise<SummaryNode[]> {
    try{
      ...
      console.error(`Failed to get summary stack items: ${err}`);
      throw new LocalDBError("Failed to get summary stack items")
  ...
  async popSummaryNode(node: SummaryNode) {
    try{
      ...
      console.error(`Failed to pop summary node: ${err}`);
      throw new LocalDBError('Failed to pop summary node');
    }
  }
  ...
  async upsertSummaryStack(chatId: string, summary: string){
    try{
      ...
      console.error(`Failed to upsert summary stack: ${err}`);
      throw new LocalDBError("Failed to upsert summary stack");
    }
  }
  ...
  async getSummary(chatId: string): Promise<SummaryModel | null> {
    ...
    return summary[0];
      ...
      console.log(`Failed to get summary of summary stack: ${err}`);
      throw new LocalDBError("Failed to get summary of summary stack");
    }
  }
}
